...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (declared > MAX_AUDIO_BYTES)
      return Response.json(
        { error: 'Audio exceeds the 5 MiB limit.' },
        { status: 413 },
      );
    const contentType = request.headers.get('content-type') ?? '';
    if (!contentType.startsWith('audio/'))
      return Response.json(
        { error: 'An audio content type is required.' },
        { status: 415 },
      );
    const audio = await request.arrayBuffer();
    if (!audio.byteLength || audio.byteLength > MAX_AUDIO_BYTES) {
      return Response.json(
        { error: 'Audio must contain 1 byte to 5 MiB.' },
        { status: 400 },
      );
    }
    const transcription = await transcribeAudio(audio, contentType, config);

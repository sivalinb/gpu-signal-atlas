...
for (const testCase of evaluationCases) {
  ...
  if (refused !== testCase.shouldRefuse) failures.push(`${testCase.id}: refusal expected=${testCase.shouldRefuse} actual=${refused}`);
}

const sortedLatency = [...latencies].sort((left, right) => left - right);
const percentile = (fraction: number) => sortedLatency[Math.min(sortedLatency.length - 1, Math.floor(sortedLatency.length * fraction))];

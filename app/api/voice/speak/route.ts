  ...
  try {
    ...
    if (!text || text.length > 3_000)
      return Response.json(
        { error: 'Briefing text must contain 1–3,000 characters.' },
        { status: 400 },
      );
    const config = getDeepgramConfig();
    if (!config)
      return Response.json(
        { error: 'Spoken briefing is not configured.' },
        { status: 503 },
      );
    ...
    if (error instanceof DeepgramError)
      return Response.json(
        { error: 'Spoken briefing failed safely.' },
        { status: 503 },
      );
    return Response.json(
      { error: 'Spoken briefing is unavailable.' },
      { status: 503 },
    );
  }
}

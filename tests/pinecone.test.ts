...
test('Pinecone dense candidates feed the existing hybrid reranker and evidence gate', async () => {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    return Response.json({
      ...
      matches: corpus.map((document, index) => ({
        id: document.id,
        score:
          document.id === 'nvidia-xid-79'
            ? 0.98
            : Math.max(0.01, 0.7 - index * 0.02),
      })),
    });
  };
  ...
  assert.ok(
    analysis.citations.every((citation) =>
      retrieval.some((item) => item.document.id === citation.id),
    ),
  );
});
...
test('Pinecone corpus sync preserves stable IDs, vectors, metadata, and namespace', async () => {
  let requestBody:
    | { namespace?: string; vectors?: Array<Record<string, unknown>> }
    | undefined;
  const fetchImpl: typeof fetch = async (input, init) => {
  ...
  assert.equal(
    await upsertCorpusToPinecone(config, corpus, fetchImpl),
    corpus.length,
  );
  assert.equal(requestBody?.namespace, config.namespace);
  ...
  assert.equal(
    (requestBody?.vectors?.[0]?.metadata as Record<string, unknown>)
      .curatedContentHash,
